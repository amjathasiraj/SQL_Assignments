...
-- populate the tables using DML insert codes
...
-- OR --> if it gets rejected due to safe update mode

select employee_id from employees where designation is null;
...
select * from employees where hire_date like '2018%' order by hire_date asc limit 5;
...
select designation, avg(salary) as avg_salary
 ...
 select department_id, count(*) as no_employees
 from employees
 ...
 select d.department_name, d.department_id, count(e.employee_id) as no_employees
 from departments d
